...
import { FaWaveSquare, FaRobot, FaCheckCircle, FaSearch, FaMicrochip } from 'react-icons/fa';
import { useEffect, useState } from 'react';

const steps = [
  { id: 1, text: "Metadata Analysis", icon: <FaSearch /> },
  { id: 2, text: "Noise & Frequency Scan", icon: <FaWaveSquare /> },
  { id: 3, text: "Voice Naturalness Check", icon: <FaMicrochip /> },
  { id: 4, text: "AI Synthetic Detection", icon: <FaRobot /> },
  { id: 5, text: "Integrity Verification", icon: <FaCheckCircle /> }
];
...
const ScannerOverlay = ({ isScanning, onComplete }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex flex-col items-center justify-center bg-black/90 backdrop-blur-md px-4">
      ...
      <h2 className="text-xl md:text-2xl font-bold text-white mb-6 tracking-widest animate-pulse text-center">
        ...
        {steps.map((step, index) => (
          <motion.div 
            ...
            animate={{ 
              ...
              color: index < currentStep ? '#00ff9d' : (index === currentStep ? '#fff' : '#444')
            }}
            className="flex items-center space-x-4 text-base md:text-lg font-mono p-2 border-b border-gray-800"
          >
            <span className={index <= currentStep ? "text-neon-green" : "text-gray-600"}>
